...
<div class="flex justify-center items-center h-full py-3">
@if($it->QR_Code)
<a href="{{ route('maquinaria.qr', $it->id) }}"
   class="action-btn border border-[var(--color-dorado)] text-[var(--color-dorado)]">
<div class="logo-wrap">
...
<a href="{{ route('maquinaria.publico', $it->id) }}"
   target="_blank"
...
<a href="{{ route('maquinaria.edit', $it->id) }}"
...
<form method="POST"
      action="{{ route('maquinaria.destroy', $it->id) }}"
