...
<head>
    ...
    <title>Ficha Equipo de Comunicación #{{ $item->id }}</title>
    @vite(['resources/css/inventario.css', 'resources/js/app.js', 'resources/js/inventario.js'])
</head>
<body>
...
<div class="glass-card">
    ...
    <div class="flex items-center justify-between mb-10">
        <div>
            <h1 class="text-3xl font-bold" style="color: var(--color-dorado);">
                Ficha Equipo de Comunicación
            </h1>
            <p style="color: var(--color-carbon-400);">
                Registro institucional del activo tecnológico
            </p>
        </div>

        <div class="text-right">
            <div class="uppercase text-xs tracking-wider" style="color: var(--color-carbon-400);">
                Registro ID
            </div>
            <div class="text-3xl font-bold mt-1" style="color: var(--color-dorado);">
                #{{ $item->id }}
            </div>
        </div>
    </div>
    ...
    <div class="grid md:grid-cols-4 gap-4 mb-8 text-sm">
        ...
        <div class="field-box">
            ...
            <div style="{{ $value }}">
                ${{ number_format(($item->MOI ?? 0) * ($item->cantidad ?? 0),2) }}
            </div>
        </div>
    ...
    <div class="flex gap-6 pt-6 border-t"
        ...
        @if($item->factura_path)
            <a href="{{ asset('storage/'.$item->factura_path) }}"
               target="_blank"
               class="btn-primary">
                Ver Factura
            </a>
        ...
        @if($item->QR_Code)
            <a href="{{ route('comunicacion.qr', $item->id) }}"
               class="btn-secondary">
                Ver QR
            </a>
        @endif
