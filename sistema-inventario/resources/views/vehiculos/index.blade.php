...
<div class="flex justify-center items-center h-full py-3">
@if($it->QR_Code)
<a href="{{ route('vehiculos.qr', $it->id) }}"
   class="action-btn border border-[var(--color-dorado)] text-[var(--color-dorado)]">
<div class="logo-wrap">
...
<a href="{{ route('vehiculos.publico', $it->id) }}"
   target="_blank"
...
<a href="{{ route('vehiculos.edit', $it->id) }}"
...
<form method="POST"
      action="{{ route('vehiculos.destroy', $it->id) }}"
